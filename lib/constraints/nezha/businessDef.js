'use strict';

var _defineProperty2 = require('babel-runtime/helpers/defineProperty');

var _defineProperty3 = _interopRequireDefault(_defineProperty2);

var _report, _factory, _certificate;

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

/**
 * Created by Administrator on 2019/9/20.
 */
var _require = require('../../constants/roleConstant2'),
    Roles = _require.Roles;

var _require2 = require('../../constants/nezha/report'),
    ReportAction = _require2.action,
    ReportRelation = _require2.relation,
    ReportState = _require2.state,
    REPORT_CONSTANTS = _require2.CONSTANTS,
    REPORT_STATE_TRAN_MATRIX = _require2.STATE_TRAN_MATRIX;

var ErrorCode = require('../../constants/nezha/errorCode');

var _require3 = require('../../constants/action'),
    COMMON_STATE_TRAN_MATRIX = _require3.COMMON_STATE_TRAN_MATRIX,
    CommonAction = _require3.action,
    CommonRelation = _require3.relation;

function genOwnerOrFactoryOwner(checkRow) {
    return {
        auths: [{
            '#relation': { // 表示现有对象与user的关系
                attr: 'machine.factory', // 这里的attr指对象（agency)中的属性，通过Schema反查找，如果没有则是指自身，即使用id属性
                relations: [ReportRelation.owner] // 如果没有relations，则任何关系都可以
            },
            '#data': [// 表示对现有对象或者用户的数据有要求，可以有多项，每项之前是AND的关系
            {
                check: function check(_ref) {
                    var user = _ref.user,
                        row = _ref.row;
                    return checkRow({ user: user, row: row });
                }
            }]
        }, {
            '#relation': { // 表示现有对象与user的关系
                relations: [ReportRelation.owner] // 如果没有relations，则任何关系都可以
            },
            '#data': [// 表示对现有对象或者用户的数据有要求，可以有多项，每项之前是AND的关系
            {
                check: function check(_ref2) {
                    var user = _ref2.user,
                        row = _ref2.row;
                    return checkRow({ user: user, row: row });
                }
            }]
        }]
    };
}

function genWorker(checkRow) {
    return {
        auths: [{
            '#relation': { // 表示现有对象与user的关系
                relations: [ReportRelation.worker] // 如果没有relations，则任何关系都可以
            },
            '#data': [// 表示对现有对象或者用户的数据有要求，可以有多项，每项之前是AND的关系
            {
                check: function check(_ref3) {
                    var user = _ref3.user,
                        row = _ref3.row;
                    return checkRow({ user: user, row: row });
                }
            }]
        }]
    };
}

var AUTH_MATRIX = {
    report: (_report = {}, (0, _defineProperty3.default)(_report, ReportAction.confirm, {
        auths: [{
            '#relation': { // 表示现有对象与user的关系
                attr: 'machine.factory', // 这里的attr指对象（agency)中的属性，通过Schema反查找，如果没有则是指自身，即使用id属性
                relations: [ReportRelation.owner] // 如果没有relations，则任何关系都可以
            },
            '#data': [// 表示对现有对象或者用户的数据有要求，可以有多项，每项之前是AND的关系
            {
                check: function check(_ref4) {
                    var user = _ref4.user,
                        row = _ref4.row;

                    return row.state === ReportState.init;
                }
            }]
        }]
    }), (0, _defineProperty3.default)(_report, ReportAction.accept, {
        auths: [{
            '#exists': [{
                relation: 'userCertificate',
                condition: function condition(_ref5) {
                    var user = _ref5.user;

                    return {
                        userId: user.id
                    };
                }
            }],
            '#data': [// 表示对现有对象或者用户的数据有要求，可以有多项，每项之间是AND的关系
            {
                check: function check(_ref6) {
                    var user = _ref6.user,
                        row = _ref6.row;

                    return row.state === ReportState.delivered;
                }
            }]
        }]
    }), (0, _defineProperty3.default)(_report, ReportAction.deliverAgain, genOwnerOrFactoryOwner(function (_ref7) {
        var user = _ref7.user,
            row = _ref7.row;

        // 前端 传入row时 因为解构 _createAt_ 变成了 createAt, 进行容错处理
        return (row._createAt_ || row.createAt) < Date.now() - REPORT_CONSTANTS.allowDeliverGap && row.state === ReportState.accepted;
    })), (0, _defineProperty3.default)(_report, ReportAction.giveUp, genWorker(function (_ref8) {
        var user = _ref8.user,
            row = _ref8.row;

        return row.state === ReportState.accepted;
    })), (0, _defineProperty3.default)(_report, ReportAction.cancel, genOwnerOrFactoryOwner(function (_ref9) {
        var user = _ref9.user,
            row = _ref9.row;

        return (row._createAt_ || row.createAt) < Date.now() - REPORT_CONSTANTS.allowDeliverGap && row.state === ReportState.accepted || [ReportState.init, ReportState.delivered].includes(row.state);
    })), (0, _defineProperty3.default)(_report, ReportAction.startRepairing, genWorker(function (_ref10) {
        var user = _ref10.user,
            row = _ref10.row;

        return [ReportState.accepted, ReportState.inRedoing, ReportState.paused].includes(row.state);
    })), (0, _defineProperty3.default)(_report, ReportAction.endRepairing, genOwnerOrFactoryOwner(function (_ref11) {
        var user = _ref11.user,
            row = _ref11.row;

        return [ReportState.inRepairing].includes(row.state);
    })), (0, _defineProperty3.default)(_report, ReportAction.commit, genWorker(function (_ref12) {
        var user = _ref12.user,
            row = _ref12.row;

        return [ReportState.inRepairing, ReportState.committed].includes(row.state);
    })), (0, _defineProperty3.default)(_report, ReportAction.admit, genOwnerOrFactoryOwner(function (_ref13) {
        var user = _ref13.user,
            row = _ref13.row;

        return [ReportState.committed].includes(row.state);
    })), (0, _defineProperty3.default)(_report, ReportAction.pauseRepairing, genWorker(function (_ref14) {
        var user = _ref14.user,
            row = _ref14.row;

        return [ReportState.inRepairing].includes(row.state);
    })), (0, _defineProperty3.default)(_report, ReportAction.rejectRepairing, genWorker(function (_ref15) {
        var user = _ref15.user,
            row = _ref15.row;

        return [ReportState.askingForRestart].includes(row.state);
    })), (0, _defineProperty3.default)(_report, ReportAction.askForRestart, genOwnerOrFactoryOwner(function (_ref16) {
        var user = _ref16.user,
            row = _ref16.row;

        return row.state === ReportState.done;
    })), (0, _defineProperty3.default)(_report, ReportAction.restart, genWorker(function (_ref17) {
        var user = _ref17.user,
            row = _ref17.row;

        return row.state === ReportState.askingForRestart;
    })), (0, _defineProperty3.default)(_report, ReportAction.surrender, genWorker(function (_ref18) {
        var user = _ref18.user,
            row = _ref18.row;

        return [ReportState.inRepairing, ReportState.inRedoing].includes(row.state);
    })), _report),
    machine: (0, _defineProperty3.default)({}, CommonAction.update, {
        auths: [{
            '#relation': {
                attr: 'factory'
            }
        }]
    }),
    factory: (_factory = {}, (0, _defineProperty3.default)(_factory, CommonAction.update, {
        auths: [{
            '#relation': {}
        }]
    }), (0, _defineProperty3.default)(_factory, CommonAction.transfer, {
        auths: [{
            '#relation': {
                relations: [CommonRelation.owner]
            }
        }]
    }), _factory),
    certificate: (_certificate = {}, (0, _defineProperty3.default)(_certificate, CommonAction.update, {
        auths: [{
            '#relation': {
                relations: [CommonRelation.owner]
            }
        }]
    }), (0, _defineProperty3.default)(_certificate, CommonAction.apply, {
        auths: [{
            '#relation': {}
        }]
    }), (0, _defineProperty3.default)(_certificate, CommonAction.reject, {
        auths: [{
            '#role': [Roles.SYSTEM_MANAGER.name]
        }]
    }), (0, _defineProperty3.default)(_certificate, CommonAction.agree, {
        auths: [{
            '#role': [Roles.SYSTEM_MANAGER.name]
        }]
    }), _certificate)
};

var STATE_TRAN_MATRIX = {
    certificate: COMMON_STATE_TRAN_MATRIX,
    report: REPORT_STATE_TRAN_MATRIX
};

module.exports = {
    AUTH_MATRIX: AUTH_MATRIX,
    STATE_TRAN_MATRIX: STATE_TRAN_MATRIX
};