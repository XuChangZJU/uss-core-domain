...
var AUTH_MATRIX = {
    ...
    })), _defineProperty(_report, ReportAction.commit, genWorker(function (_ref12) {
        var user = _ref12.user,
            row = _ref12.row;

        return [ReportState.inRepairing, ReportState.committed].includes(row.state);
    })), _defineProperty(_report, ReportAction.admit, genOwnerOrFactoryOwner(function (_ref13) {
        var user = _ref13.user,
            row = _ref13.row;

        return [ReportState.committed].includes(row.state);
    })), _defineProperty(_report, ReportAction.pauseRepairing, genWorker(function (_ref14) {
        var user = _ref14.user,
            row = _ref14.row;

        return [ReportState.inRepairing].includes(row.state);
    })), _defineProperty(_report, ReportAction.rejectRepairing, genWorker(function (_ref15) {
        var user = _ref15.user,
            row = _ref15.row;

        return [ReportState.askingForRestart].includes(row.state);
    })), _defineProperty(_report, ReportAction.askForRestart, genOwnerOrFactoryOwner(function (_ref16) {
        var user = _ref16.user,
            row = _ref16.row;

        return row.state === ReportState.done;
    })), _defineProperty(_report, ReportAction.restart, genWorker(function (_ref17) {
        var user = _ref17.user,
            row = _ref17.row;

        return row.state === ReportState.askingForRestart;
    })), _defineProperty(_report, ReportAction.surrender, genWorker(function (_ref18) {
        var user = _ref18.user,
            row = _ref18.row;

        return [ReportState.inRepairing, ReportState.inRedoing].includes(row.state);
    })), _report),
    machine: _defineProperty({}, CommonAction.update, {
        auths: [{
            '#relation': {
                attr: 'factory'
            }
    ...
    factory: (_factory = {}, _defineProperty(_factory, CommonAction.update, {
        auths: [{
            '#relation': {}
        }]
    }), _defineProperty(_factory, CommonAction.transfer, {
        auths: [{
            '#relation': {
    ...
    certificate: (_certificate = {}, _defineProperty(_certificate, CommonAction.update, {
        auths: [{
            '#relation': {
                relations: [CommonRelation.owner]
            }
        }]
    }), _defineProperty(_certificate, CommonAction.apply, {
        auths: [{
            '#relation': {}
        }]
    }), _defineProperty(_certificate, CommonAction.reject, {
        auths: [{
            '#role': [Roles.SYSTEM_MANAGER.name]
        }]
    }), _defineProperty(_certificate, CommonAction.agree, {
        auths: [{
            '#role': [Roles.SYSTEM_MANAGER.name]
        }]
    }), _certificate)
