...
function dateAdd(scale, number, date) {
    var date2 = date instanceof Date ? new Date(date.valueOf()) : new Date(date);
    switch (scale.toLowerCase()) {
        case "y":
    ...
    if (scale2 === "y") {
        ...
        if (scale2 === "m") {
            ...
            if (scale2 === 'w') {
                 ...
                 */
                if (left) {
                    var diff = 1 - dayOfWeek;
                    if (diff > 0) {
                        diff = diff - 7;
                    }
                    date2.setDate(dayOfMonth + diff);
                } else {
                    var _diff = 8 - dayOfWeek;
                    if (_diff > 7) {
                        _diff = _diff - 7;
                    }
                    date2.setDate(dayOfMonth + _diff);
                }
            } else {
                if (scale2 === "d") {
                    if (left) {
...
module.exports = {
    ...
    getWeek: getWeek,
    align: align,
    transferPrsToUnit: transferPrsToUnit
};