...
function modifySchema(schema) {
    if (schema.type instanceof Array) {
        schema.type = schema.type.find(ele => ele !== 'null');
    }
    ...
    if (schema.type === 'object') {
        ...
        keys(schema.properties).forEach(k => {
...
function modifyData(data, schema) {
    if (data instanceof Array) {
        data.forEach(ele => modifyData(ele, schema.items));
        ...
        keys(data).forEach(k => {
            // 使所有id项和对应的对象id对应
            if (typeof data[k] === 'object' && data[k] && data[k].hasOwnProperty('id') && data.hasOwnProperty(`${k}Id`) && typeof data[`${k}Id`] === 'number') {
                data[`${k}Id`] = data[k].id;
            }
        });
