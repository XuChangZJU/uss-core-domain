...
function modifyData(data, schema) {
    if (data instanceof Array) {
        ...
        keys(data).forEach(function (k) {
            // 使所有id项和对应的对象id对应
            if (_typeof(data[k]) === 'object' && data[k] && data[k].hasOwnProperty('id') && data.hasOwnProperty(k + 'Id') && typeof data[k + 'Id'] === 'number') {
                data[k + 'Id'] = data[k].id;
            }
        });
        ...
        keys(data).forEach(function (k) {
            // 使所有id项和对应的对象id对应
            if (_typeof(data[k]) === 'object' && data[k]) {
...
var generateDataBySchema = function generateDataBySchema(schema, tailor) {
    ...
    if (tailor) {
        if ((typeof tailor === 'undefined' ? 'undefined' : _typeof(tailor)) === 'object') {
